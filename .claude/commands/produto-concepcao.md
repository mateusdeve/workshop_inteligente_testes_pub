...
Leia `entregas/.ativo` para obter o produto ativo. Leia `entregas/{ativo}/perfil.md`. Se existir, mostre resumo e pergunte se quer atualizar.
...
**Detecção de tipo de produto:** Verifique no `perfil.md` se o produto é de entrada (low ticket, R$37-97) ou se o formato é planilha, checklist, e-book, agente GPT, template ou desafio. Se for produto de entrada, siga a regra abaixo. Se for produto regular (middle ou high ticket), siga o fluxo padrão com macroetapas.

**Se for produto de entrada (low ticket):** A Furadeira É o próprio produto/ferramenta. Não pergunte sobre macroetapas. Pergunte: "Qual ferramenta o comprador vai receber? O que ela faz? Como ele usa no dia a dia?" Registre no perfil: "A ferramenta é a Furadeira. [tipo da ferramenta + o que resolve + como o comprador usa]". Depois siga para o Bloco 3.

**Se for produto regular (middle/high ticket):**

A Furadeira é o mecanismo único do método. Não é só uma lista de etapas. É o que torna visível a **eficiência** do produto (eficaz cumpre o prometido, eficiente cumpre melhor: mais rápido, mais barato, com menos esforço, menos dor, mais adesão, etc.). Antes de montar, lembre o aluno: "apaixone-se pelo problema, não pela solução".

Pergunte como o aluno ensina/entrega o resultado. quais as grandes fases do processo. Pergunte UMA coisa por vez.

Com a resposta, monte a Furadeira completa aplicando as **6 mecânicas** (consulte `.claude/plugins/workshop-marketing/skills/furadeira-visual/references/6-mecanicas.md` para o detalhamento). Não precisa usar as 6, escolha as que combinam com o método:
...
5. **Identificando Empecilhos**. Quais os impedimentos reais que atrapalham o público e como o método ajuda a vencer cada um? (Princípio: se a vida fosse perfeita, o produto não existia.)
...
- Cuidado com furadeira mirabolante. O objetivo é gerar sensação de facilidade, não complexidade. Se passou de 7 macroetapas, provavelmente é gordura. Se o nicho exige profundidade, quebre em fases.
...
- Pergunte a paleta (1 pergunta numerada)
- Gere o HTML com o template de trilha progressiva
- Salve em `produtos/{ativo}/entregas/furadeira-visual.html`
- Tente capturar PNG via Chrome se disponível
- Confirme o caminho e avise que pode continuar

Só siga para o Bloco 3 após confirmar que o arquivo foi salvo.

**Bloco 3/6. Pesquisa de Mercado + Identidades e Posicionamento:**

Pergunte brevemente quem é o público-alvo do aluno.

**Acione a skill `pesquisa-mercado` OBRIGATORIAMENTE agora.** Essa skill faz uma pesquisa completa (tamanho de mercado, mínimo 10 concorrentes com tabela, faixa de preço, público real, objeções do Reclame Aqui, SEBRAE, ângulos virais, biblioteca de anúncios, riscos regulatórios) e salva o relatório em `entregas/{ativo}/pesquisa-mercado.md`. Não pule, não faça versão abreviada, não substitua por uma WebSearch solta. A pesquisa é o que transforma sugestão em decisão fundamentada.

Enquanto a pesquisa roda, avise o aluno: "Vou fazer uma pesquisa de mercado completa agora. Visitando Reclame Aqui, SEBRAE, concorrentes e fontes do nicho. Leva alguns minutos."

Quando a pesquisa terminar, apresente um resumo conversacional dos achados (dados, números, insights principais, não o relatório inteiro) e use os dados para gerar as 3 Identidades:
...
Não apenas pergunte. Use a sugestão de preço que já saiu na pesquisa (seção 3 do relatório) e explique o raciocínio apoiado nos concorrentes mapeados. Se o aluno discordar, argumente com os dados da pesquisa e ofereça alternativas em faixas diferentes, explicando o posicionamento de cada uma.
...
**Se geração automática:** Gere as 7 categorias completas (10 itens cada) com base em tudo que já coletou (Quadro, Identidades, pesquisa). Apresente para validação e ajuste.
...
Salve em `entregas/{ativo}/perfil.md` com a estrutura:
...
### 5. Gerar PDF (obrigatório)

Após salvar o `perfil.md`, gere também uma versão em PDF em `entregas/{ativo}/perfil.pdf` com exatamente o mesmo conteúdo. Use a skill `anthropic-skills:pdf` para converter o markdown em PDF formatado (títulos, subtítulos, listas e seções preservados). O PDF deve ser legível, limpo e pronto para o aluno abrir, imprimir ou compartilhar.

Não mostre código ao usuário. Apenas gere o arquivo silenciosamente e confirme o caminho no passo 6.

### 6. Próximo Passo
"Perfil salvo em `entregas/{ativo}/perfil.md` e `entregas/{ativo}/perfil.pdf`. Agora use `/produto-consumidor` para detalhar a identidade do consumidor (cliente ideal), ou `/copy-pagina` se quiser ir direto para a página."
