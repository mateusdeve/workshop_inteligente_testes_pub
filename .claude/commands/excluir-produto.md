---
name: workshop-marketing:excluir-produto
...
1. Apague toda a pasta `produtos/{slug}/` e seu conteúdo recursivamente usando o Bash:
   ```bash
   rm -rf "produtos/{slug}"
   ```

2. **Se o produto excluído era o produto ativo** (slug igual ao conteúdo de `produtos/.ativo`):

   - Liste os produtos restantes em `produtos/`
   - **Se ainda houver outros produtos:** pergunte qual ativar:
     ...
     Digite o número:
     ```
     Após a escolha, salve o slug em `produtos/.ativo`.

   - **Se não houver mais produtos:** apague o conteúdo de `produtos/.ativo` (deixe o arquivo vazio).
...
Use /meu-produto para editar o perfil ou /trocar-produto para alternar entre produtos.
```
...
Use /novo-produto para criar um novo produto.
```
