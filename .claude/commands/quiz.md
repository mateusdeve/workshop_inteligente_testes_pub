...
description: Gerar quiz de vendas completo para o Caixa Rápido — 10 perguntas (estrutura SPIN) + prompt técnico para o Lovable.dev, entregues em um único arquivo .md pronto para colar.
---

# Quiz de Vendas — Caixa Rápido
...
O arquivo é salvo em `produtos/{ativo}/entregas/quiz/quiz-[produto].md` e está pronto para colar diretamente no Lovable.dev.
...
O quiz é a ponte entre o anúncio e a venda do produto de entrada (Caixa Rápido). O lead responde exatamente 10 perguntas sobre sua situação atual, dores e desejos, recebe um "diagnóstico personalizado" e é direcionado para a página final com a oferta do produto.
...
Se `perfil.md` não existir, oriente a usar `/meu-produto` primeiro.
...
3. **Quadro Comparativo Antes × Depois** — logo após o subtítulo, em formato de tabela comparativa
...
> ⚠️ A Página Final de Oferta do quiz **não tem seção de vídeo**. Diferente da `/pagina-de-vendas`, aqui o lead já foi aquecido pelo quiz — não é necessário vídeo de vendas.
...
- O Quadro Comparativo fica obrigatoriamente logo após o subtítulo
...
| Antes | Depois |
|---|---|
| [dor concreta] | [resultado concreto] |
| ... | ... |
...
`produtos/{ativo}/entregas/quiz/quiz-[produto].md`
