...
GET /{pixel_id}/stats?aggregation=event&start_time={now-7d}&end_time={now}
...
A primeira chamada confirma metadata. A segunda traz contagem por evento por dia (gráfico). A terceira traz totais. A quarta traz EMQ se disponível.
